...
func init() {
	android.RegisterModuleType("epic_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = epicExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = epicExpandVariables(ctx, dep_file)
	...
	cmd := epicExpandVariables(ctx, String(g.properties.Cmd))
