...
cat <<EOF
Additional EpicROM functions:
- cout:            Changes directory to out.
- mmp:             Builds all of the modules in the current directory and pushes them to the device.
- mmap:            Builds all of the modules in the current directory and its dependencies, then pushes the package to the device.
- mmmp:            Builds all of the modules in the supplied directories and pushes them to the device.
- epicgerrit:   A Git wrapper that fetches/pushes patch from/to EpicROM Gerrit Review.
- epicrebase:   Rebase a Gerrit change and push it again.
- epicremote:   Add git remote for EpicROM Gerrit Review.
- aospremote:      Add git remote for matching AOSP repository.
- cloremote:       Add git remote for matching CodeLinaro repository.
- githubremote:    Add git remote for EpicROM Github.
...
function epicremote()
{
    ...
    local EPIC_USER=$(git config --get review.review.epicos.org.username)
    if [ -z "$EPIC_USER" ]
    then
        git remote add epic ssh://review.epicos.org:29418/$PFX$PROJECT
    else
        git remote add epic ssh://$EPIC_USER@review.epicos.org:29418/$PFX$PROJECT
    fi
...
function githubremote()
{
    ...
    git remote add github https://github.com/EpicROM/$PROJECT
...
function epicgerrit() {
    ...
    local user=`git config --get review.review.epicos.org.username`
...
function epicrebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "EpicROM Gerrit Rebase Usage: "
    ...
    git fetch "http://review.epicos.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
